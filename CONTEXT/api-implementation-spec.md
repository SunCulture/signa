...
- E-signature/personalization parity note: Signa stores company logo as an account-scoped storage attachment and PKCS#12/PFX signing certificate settings in `encrypted_configs`, matching DocuSeal's account-level settings boundary. Certificate-backed PDF signing is implemented with `@signpdf/signer-p12` and defaults to the PAdES `ETSI.CAdES.detached` SubFilter through `@signpdf/utils`. Timestamp server URLs are optional, validated with real RFC3161 timestamp queries before persistence, and can be made fail-closed with `PDF_TIMESTAMP_REQUIRED=true`; completed PDFs append an RFC3161 `/DocTimeStamp` dictionary with `/SubFilter /ETSI.RFC3161` when a TSA URL is configured, and timestamp metadata is stored on completed-document rows. PDF verification uses PKI.js (`pkijs`, `asn1js`) for RFC5652 CMS `SignedData` parsing, signed-attribute/messageDigest validation against the PDF ByteRange, CMS signature verification, account-uploaded trust roots in `pdf_trust_roots`, validity-window checks, CA/basic-constraints checks, and Signa trust-root chain classification. Completed signing now attempts OCSP first and CRL fallback for external signer certificates that publish AIA/CRL distribution URLs, stores raw evidence in `pdf_revocation_evidence`, and appends DSS/VRI through a low-level incremental update that preserves signed bytes. Signa autogenerated self-host certificates generate issuer-signed internal CRL evidence, embed it into DSS/VRI, and verify as `revocation_status=good` plus `ltv_status=valid`; `PDF_LTV_REQUIRED=true` fails closed unless good evidence is collected and embedded. PDF/A output is implemented as an optional Ghostscript/veraPDF-backed pre-signing adapter controlled by `PDF_A_ENABLED`/`PDF_A_REQUIRED`.
...
- Signa-generated PDF LTV is implemented with PAdES signing, DSS/VRI evidence embedding, internal CRL evidence for autogenerated Signa certificates, optional final RFC3161 document timestamps, optional Ghostscript/veraPDF-backed PDF/A conversion before signing, account-uploaded external trust roots, and verifier-side `ltv_status=valid`. Remaining PDF hardening is focused on broader HexaPDF/PDFium edge cases and more exhaustive OCSP responder-chain validation.
...
- Timestamp/LTV/PDF-A parity status: optional RFC3161 timestamp server validation and `/DocTimeStamp` embedding are implemented. Existing signing continues when no TSA URL is configured; regulated deployments can set `PDF_TIMESTAMP_REQUIRED=true` to fail closed when timestamp evidence cannot be embedded. Verification parses CMS `SignedData`, validates the signed `messageDigest` attribute against the PDF ByteRange bytes, verifies the CMS signature over signed attributes, extracts signer certificates, classifies trusted/external/expired/invalid/missing chains using Signa roots plus account-uploaded trust roots, validates matching DSS/VRI revocation evidence where present, and returns `ltv_status=valid|missing|invalid`. Completed PDFs now attempt OCSP/CRL collection and incremental DSS/VRI embedding without rewriting signed bytes. PDF/A conversion/validation is available before signing through optional Ghostscript/veraPDF adapters.
...
- Remaining gap: generated PDFs are stamped/serialized with the currently supported Signa field set; advanced DocuSeal rendering details such as exact font/layout parity, deeper PDF/A deployment validation fixtures, and broader HexaPDF/PDFium edge cases remain pending.
...
- Supports JSON base64/URL documents, multipart uploads, provided coordinate fields, standard AcroForm extraction, XFA field extraction through PDF.js' normalized XFA tree, preview generation, and submitter creation.
...
- Pending: safer embedded `{{...}}` text-tag removal/extraction for difficult content streams. XFA upload support is implemented by rasterizing unsupported XFA visual content with PDF.js and extracting editable XFA controls into DocuSeal-style normalized fields before storage.
...
- Remaining PDF parity gaps: broader annotation-preservation fixtures, rotated-page normalization fixtures, exact typography/layout parity, deployment-specific PDF/A validation fixtures, and more exhaustive OCSP responder-chain validation.
