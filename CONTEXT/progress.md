...
- Re-audited Signa backend routes against the local DocuSeal OpenAPI file on 2026-06-30:
  ...
  - current parity gaps are behavioral, not missing endpoint routes: DOCX geometry extraction, full DSS/VRI LTV verification, and provider-backed payment completion;
...
- PDF template/signing gaps:
  ...
  - PDF signature verification currently validates parseability, Signa completion checksum matches using stored completed-document SHA-256 values, embedded signature metadata, PAdES SubFilter status, embedded RFC3161 document timestamp signature presence, structural ByteRange SHA-256 coverage, RFC5652 CMS signed attributes/messageDigest against the ByteRange bytes, CMS signature validity, certificate-chain extraction/classification as Signa-trusted, external, or missing, matching DSS/VRI entries, and embedded OCSP/CRL evidence status. Remaining LTV hardening: stronger chain-policy validation against uploaded customer trust roots, more exhaustive OCSP responder-chain validation, and PDF/A conversion/validation.
