...
- Re-audited Signa backend routes against the local DocuSeal OpenAPI file on 2026-06-30:
  - all DocuSeal public API routes in the local OpenAPI spec are implemented under Signa's `/api` prefix;
  - current parity gaps are behavioral, not missing endpoint routes: DOCX geometry extraction edge cases, PDF/A conversion/validation, and provider-backed payment completion;
  - recorded the endpoint list and caveats in `CONTEXT/api-implementation-spec.md`.
- Started a Swagger documentation hardening pass:
  - public DocuSeal-compatible template, submission, submitter, webhook, and tools controllers now have useful `@ApiOperation` summaries/descriptions and important path/query metadata;
  - high-use DTO fields for submission/template creation, submitter updates, webhook management, and PDF merge/verify now include concrete examples and field descriptions suitable for external API users.
- Extended Swagger coverage beyond public endpoints:
  - authenticated account, user, team, health, realtime, storage, signing, provider callback, submitter tracking, and system controllers now include operation descriptions, path/query metadata, and request body descriptions where relevant;
  - start-form, signing, template audit/version/folder, team, health, and tools DTOs now include clearer field-level examples and descriptions.
...
- Added DocuSeal-style `/settings/e-signature` and `/settings/personalization` pages:
  ...
  - E-Signature page supports signed PDF verification via `POST /api/tools/verify`, PKCS#12/PFX signing certificate upload/list/default/remove backed by `encrypted_configs`, Signa autogenerated self-host certificate fallback, optional RFC3161 timestamp server URL validation/persistence, and PDF result preference controls;
...
- Implemented XFA positional field extraction for uploaded template PDFs using PDF.js' normalized XFA tree. XFA controls are mapped into the same DocuSeal-style normalized areas as AcroForm fields, including date/phone heuristics and multiline text preferences, before the XFA document is rasterized for reliable browser/server previews.
...
- Added owner auto-sign foundations for programmatic submissions:
  - account e-signature preferences can enable owner auto-sign, choose the owner role name, and decide whether the owner receives invitation email;
  - template preferences can inherit, force-enable, or force-disable owner auto-sign per template;
  - `POST /api/submissions`, `/api/submissions/init`, and nested `/api/templates/:id/submissions` accept `auto_sign_owner`, `auto_sign_owner_role`, and submitter-level `use_saved_signature`/`completed_by_user_id`;
  - saved signature/initials attachments are cloned onto the auto-signed submitter, required owner fields are validated, date fields are filled, the owner submitter is completed before invitations go out, and `api_complete_form` audit events record the auto-signing user.
...
- Added first DocuSeal-style generated document backend pass:
  ...
  - generated completed PDFs now persist original document UUID/SHA-256 metadata alongside result SHA-256 metadata, are cryptographically signed with the account default PKCS#12 certificate through `@signpdf/signer-p12`, append optional RFC3161 `/DocTimeStamp` evidence when a TSA URL is configured, and audit trail PDFs include DocuSeal-style original/result checksum evidence for tamper checks;
  - signer download now returns generated completed PDFs after completion instead of source template documents;
  - result PDF generation now flattens source AcroForms through the backend `@embedpdf/pdfium` adapter before stamping, falls back to `pdf-lib` flattening for malformed sources PDFium cannot process, normalizes signer image/signature assets through `sharp`, saves with desktop-reader-friendly serialization, and invalidates old generated documents when values change or older outputs lack a value hash;
...
- Added the first DocuSeal-style template/submission detail navigation flow:
  ...
  - public signing views now persist repeated `view_form` events, first `start_form` events, request IP/user-agent metadata, parsed browser/OS/device details, locale, and timezone while keeping `opened_at` as first-open state;
  ...
  - submission event-log responses now expose browser, OS, IP, and timezone metadata so dashboard timelines can show traceable signing context without opening raw event JSON.
- Added DocuSeal-style completed public signing route behavior:
  - completing a document in-session keeps the floating completion card over the document preview;
  - directly opening or refreshing a completed signing link redirects to `/s/:slug/completed`, a lightweight completion summary with send-copy, download-documents, resubmit, and attribution actions.
...
- PDF template/signing gaps:
  ...
  - Generated result PDFs stamp supported values onto pages with `pdf-lib` and are signed with `@signpdf/signer-p12`; signatures now default to the PAdES `ETSI.CAdES.detached` SubFilter through `@signpdf/utils`, with `PDF_SIGNATURE_SUBFILTER=adobe` reserved for legacy compatibility. RFC3161 timestamp server URLs are optional, validated before save, and an RFC3161 `/DocTimeStamp` dictionary is appended to completed PDFs when configured; `PDF_TIMESTAMP_REQUIRED=true` makes timestamp embedding fail-closed. Signa now attempts OCSP first and CRL fallback for external signer certificates with revocation endpoints, caches raw evidence in `pdf_revocation_evidence`, and appends DSS/VRI with a low-level incremental update that preserves signed bytes. Signa autogenerated self-host certificates now generate issuer-signed internal CRL evidence, embed it into DSS/VRI, and support `PDF_LTV_REQUIRED=true` fail-closed behavior with verifier-side `ltv_status=valid`. The final signing order is optional PDF/A conversion -> PAdES signature -> DSS/VRI append -> optional RFC3161 document timestamp. DocuSeal's broader HexaPDF edge cases, annotation preservation, and rotation fixtures are still not complete.
  - E-Signature certificate upload/list/default/remove is implemented for settings management and those certificates are used by the PDF result-signing engine. Account trust-root upload/list/remove is implemented through `pdf_trust_roots` and is used during `POST /api/tools/verify` for customer/partner certificates.
  - PDF signature verification currently validates parseability, Signa completion checksum matches using stored completed-document SHA-256 values, embedded signature metadata, PAdES SubFilter status, embedded RFC3161 document timestamp signature presence, structural ByteRange SHA-256 coverage, RFC5652 CMS signed attributes/messageDigest against the ByteRange bytes, CMS signature validity, certificate-chain extraction/classification as trusted, external, expired, invalid, or missing using Signa roots plus uploaded account trust roots, matching DSS/VRI entries, embedded OCSP/CRL evidence status, and `ltv_status=valid` for Signa-generated DSS/VRI evidence. Remaining hardening: more exhaustive OCSP responder-chain validation and wider HexaPDF/PDFium edge-case fixtures.
  - XFA upload, preview, and editable-control extraction are implemented through the PDF.js fallback path because `pdf-lib` does not support XFA.
