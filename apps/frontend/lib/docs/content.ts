...
export const guideArticles: DocsArticle[] = [
  ...
  },
  {
    ...
    steps: [
      "Copy .env.example to .env and set JWT_SECRET, API_PUBLIC_URL, FRONTEND_ORIGIN, and NEXT_PUBLIC_API_BASE_URL.",
      "Use SQLite for a single-node evaluation or PostgreSQL for production durability.",
      "Configure S3-compatible storage for production document blobs.",
      "Keep REGISTRATION_MODE=initial_only to allow only the first owner registration.",
      "Run docker compose up -d and confirm /auth/login, /api/health, and /api/docs.",
    ],
  },
  {
